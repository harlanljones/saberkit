...
- `cargo test -p saberkit-core`: **67 passed, 0 failed**, plus **1 passing
  doctest** (`ip::ip_to_outs`).
- `cargo clippy --workspace --all-targets -- -D warnings`: **clean, 0
  warnings**.
- `saberkit-py` now has a native Rust test suite: **13 passed, 0 failed**
...
- pytest baseline now verified: **53 passed** from an installed wheel in a
  clean virtual environment, up from 50 before season constants landed.
...
- Statistics implemented per `README.md`'s table: rate stats (`obp` `slg`
  ...
  `league.rs`, `ip.rs` all exist and match. `LeagueContext.for_season`
  supplies Retrosheet-derived constants for completed seasons 2010–2025.
- CI (`.github/workflows/ci.yml`) defines five jobs: `rust`, `python` (matrix
  ...
  `wheels` (the approved five-target matrix), and `sdist`. GitHub Actions run
  [32591120952](https://github.com/harlanljones/saberkit/actions/runs/32591120952)
  passed on commit `c060fac`: all five wheel targets and the sdist job
  succeeded and uploaded their six expected artifacts. RM-2b is complete.
- README's "Not yet included" list now contains two items:
  1. No PyPI release. The cross-platform build matrix is verified and
     `.github/workflows/release.yml` is prepared for trusted publishing, but
     the PyPI project/environment and first tag remain external steps.
  2. No park-factor computation — this is a deliberate permanent non-goal.
- RM-3 and RM-3b are complete. The source is Retrosheet regular-season
  play-by-play mirrored by the Chadwick Bureau at revision `ee82e50`; the
  standard-library generator, per-season SHA-256 digests, `NOTICE`, core and
  binding tests, Python facade, and 2010–2025 generated table are checked in.
...
zero-copy, zero-required-dependency binding guarantee intact while completing
...
- Publishing the already prepared first release and preserving the completed
  season-constants and wheel-build milestones.
...
- The first package-index release includes the verified cross-platform wheels,
  source distribution, and completed Retrosheet-derived season constants.
...
- **Wheel platform matrix scope: resolved.** The first release targets the five
  entries recorded under M3. The maintainer approved the proposal, and all
  five targets passed in GitHub Actions run 32591120952; RM-2b is complete.
- **Season-constants sourcing: resolved.** Constants are derived from
  Retrosheet play-by-play rather than copied from FanGraphs. Retrosheet permits
  derived products and redistribution with the attribution recorded in
  `NOTICE`; the exact mirror revision and source-file digests are recorded.

### Adopted season-constants source standard (RM-3)

This proposal is deliberately stricter than “the numbers are visible on a web
page.” A bundled dataset is accepted only when all of the following hold:

1. **Provenance and citation.** Every row traces to a named, versioned source
   or to a checked-in deterministic derivation from named raw inputs. The
   package documentation cites both the source and the retrieval/version date.
2. **Redistribution rights.** The source has an explicit license permitting
   redistribution in an MIT/Apache-licensed package, or saberkit has written
   permission covering the bundled snapshot. Public visibility and an export
   button are not treated as a license.
3. **Schema.** Each completed MLB season supplies `woba`, `woba_scale`, `w_bb`,
   `w_hbp`, `w_1b`, `w_2b`, `w_3b`, `w_hr`, `runs_per_pa`, and `c_fip` as
   finite decimals, plus `season`, source identifier, and source version/date.
   Optional source columns such as `runSB`, `runCS`, and `R/W` stay out of the
   runtime schema until a planned statistic consumes them.
4. **Historical coverage and mutability.** The first supported range must be
   contiguous and documented. In-progress seasons are excluded because their
   constants can change; adding a completed season is a reviewed data update,
   not a live network lookup.
5. **Reference validation.** Source-integrity tests verify contiguity, required
   fields, population identities, and per-season SHA-256 digests. A known 2024
   season is compared with published values at documented tolerances because
   Retrosheet-derived values intentionally use a distinct methodology and are
   not expected to equal FanGraphs exactly. Rust/Python regression tests
   exercise the same season through `LeagueContext.for_season`.

The adopted source is Retrosheet's regular-season event data for 2010–2025,
using the Chadwick Bureau mirror revision recorded in generated Rust. The
...
| --- | --- | --- | --- | --- | --- |
| `saberkit-core` unit test pass count | 67 passed / 0 failed (4 season-constant tests added) | No regression; grows with new statistics | `cargo test -p saberkit-core`, read summary line | TBD | Every PR touching `saberkit-core` |
...
- Exit gate: **Done.** Retrosheet source and attribution, 2010–2025 range,
  all-batters population, schema, generator, source digests, and validation
  rules are recorded; RM-3b shipped the generated table and public API.
...
- Exit gate: RM-1's release policy is recorded; RM-2b and RM-3b are complete;
  all configured
...
```text
RM-1 (policy) ────────────────────────────────┐
RM-2 ──> RM-2b (verified wheel CI) ───────────┼─> RM-1b (first release)
RM-3 ──> RM-3b (season constants) ────────────┘
```

RM-1b is now blocked only by maintainer-owned PyPI project/environment setup,
approval of the release commit, and creation of the matching version tag.

- **Waves A and B:** complete. Policy, wheel CI, season constants, bindings,
  attribution, packaging metadata, and the trusted-publishing workflow exist.
- **Integration gate B:** local Rust, binding, pytest, wheel-build, package
  contents, and minimal-install lanes must pass on the combined tree, followed
  by a green GitHub matrix on the release commit.
...
| RM-2b | Complete | RM-2 | CI/release agent | `.github/workflows/ci.yml` | **Done.** Multi-platform wheel and sdist build jobs | GitHub Actions run 32591120952: five wheel jobs and sdist succeeded; six artifacts uploaded | Preserve all five wheel targets and the sdist artifact in CI |
| RM-3 | Complete | none | Stats/data reviewer | `ROADMAP.md`, `NOTICE` | **Done.** Retrosheet source/attribution and derivation/validation standard | Source terms reviewed; revision and population recorded | Preserve attribution and methodology documentation |
| RM-3b | Complete | RM-3 | Cross-crate Rust/Python agent | generator, generated constants, `league.rs`, bindings, Python facade, tests, README | **Done.** `LeagueContext.for_season(year)` for 2010–2025 | 67 core tests, 13 binding tests, 53 pytest tests; source digests and known-season comparison | Preserve contiguous completed-season coverage and regeneration auditability |
| RM-1b | Blocked by external ownership | RM-1, RM-2b, and every other milestone included by policy | Maintainer (TBD who) | version metadata, release workflow, tag | First tagged package-index release | All CI jobs green on release commit; tag and package artifact exist; minimal wheel smoke test passes | Release policy is exercised end-to-end once |
| RM-4 | Complete | complete | Rust agent | binding test harness and CI Rust job | **Done.** Native binding tests and CI execution | `cargo test -p saberkit-py --no-default-features`: 13 passed, 0 failed | Preserve the suite and zero failures |
...
| --- | --- | --- |
| Zero required runtime Python dependencies | Standing release gate | Empty `[project].dependencies`; CI `minimal` job passed in run 32591120952; validate again in RM-1b |
| No bundled season constants | Closed | RM-3/RM-3b: Retrosheet-derived 2010–2025 table and `LeagueContext.for_season` |
...
- Season-constant updates run `cargo fmt --all --check`, both Rust test suites,
  clippy, pytest, the minimal-install computation, source-integrity tests, and
  a wheel/sdist contents audit because they cross core, binding, Python, data
  provenance, and packaging boundaries.
...
- For every generated-season refresh, re-verify README's "Accuracy" claims
...
| Season constants become stale or irreproducible | A season row is hand-edited, generated from unrecorded inputs, or refreshed without attribution | Generated file warns against hand edits; preserve the generator, mirror revision, per-season SHA-256 digests, Retrosheet `NOTICE`, identity tests, and known-season comparison |
...
| Release cannot publish despite green builds | PyPI project ownership or trusted-publisher environment remains unresolved when RM-1b starts | Configure the `saberkit` PyPI project and `pypi` GitHub environment for `.github/workflows/release.yml` before pushing the release tag |
